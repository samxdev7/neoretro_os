...
void funcion_campo_bloc(CampoBloc *bloc, short mouse_x, short mouse_y) {
    ...
    if (bloc -> foco && kbhit()) {
        ...
        if (tecla == 0) /* Caso: ASCII EXTENDIDO */
        {
            short tecla_extendida = getch();

            /* Ahora toma el ascii de la tecla presionada */
            switch (tecla_extendida)
            {
                case 75: /* Caso: Flecha Izquierda */
                    /* Se mueve cursor de fichero a la izquierda */
                    if (ftell(bloc -> bloc) > 0)
                        fseek(bloc -> bloc, -1, SEEK_CUR);

                    break;
                case 77: /* Caso: Flecha Derecha */    
                    /* Se mueve cursor de fichero a la derecha */
                    if (ftell(bloc -> bloc) < contar_caracteres_file(bloc -> bloc))
                        fseek(bloc -> bloc, 1, SEEK_CUR);
